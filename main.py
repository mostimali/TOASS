# ------------------------------------------------------------------
# TOASS: A Simple Network for Image Anomaly Detection and Localization (https://openaccess.thecvf.com/content/CVPR2023/papers/Liu_TOASS_A_Simple_Network_for_Image_Anomaly_Detection_and_Localization_CVPR_2023_paper.pdf)
# Github source: https://github.com/DonaldRR/TOASS
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        toass_list = methods["get_toass"](imagesize, device)

        models_dir = os.path.join(run_save_path, "models")
        os.makedirs(models_dir, exist_ok=True)
        for i, TOASS in enumerate(toass_list):
            # torch.cuda.empty_cache()
            if TOASS.backbone.seed is not None:
                utils.fix_seeds(TOASS.backbone.seed, device)
            LOGGER.info(
                "Training models ({}/{})".format(i + 1, len(toass_list))
            )
            # torch.cuda.empty_cache()

            TOASS.set_model_dir(os.path.join(models_dir, f"{i}"), dataset_name)
            if not test:
                i_auroc, p_auroc, pro_auroc = TOASS.train(dataloaders["training"], dataloaders["testing"])
            else:
                # BUG: the following line is not using. Set test with True by default.
                # i_auroc, p_auroc, pro_auroc =  TOASS.test(dataloaders["training"], dataloaders["testing"], save_segmentation_images)
...
def net(
    ...
    def get_toass(input_shape, device):
        toasss = []
        for backbone_name, layers_to_extract_from in zip(
            ...
            toasss.append(toass_inst)
        return toasss

    return ("get_toass", get_toass)
