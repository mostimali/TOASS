...
class TOASS(torch.nn.Module):
    def __init__(self, device):
        """anomaly detection class."""
        super(TOASS, self).__init__()
        self.device = device
    ...
    def load(
        ...
        feature_aggregator = common.FeatureExtractor(
            self.device, 'wide_resnet50_2', ['layer2', 'layer3'], 3, (288, 288)
        )

        feature_aggregator1 = common.NetworkFeatureAggregator(
            self.backbone, self.layers_to_extract_from, self.device, train_backbone
        )

        feature_dimensions = feature_aggregator1.feature_dimensions(input_shape)
