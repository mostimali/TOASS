...
class FeatureExtractor(torch.nn.Module):
    ...
    def get_feature_dim(self, image_shape):
        # dryrun
        _input = torch.ones([1] + list(image_shape)).to(self.device)
        # 创建一个与输入形状相同的张量 _input
        _output = self(_input)
        return [_output[layer].shape[1] for layer in self.layers]
